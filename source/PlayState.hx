...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'not a slacker anymore':
	            dialogue = CoolUtil.coolTextFile(Paths.txt('not a slacker anymore/not a slacker anymoreDialogue'));
			case 'but he refused to give up':
				dialogue = CoolUtil.coolTextFile(Paths.txt('but he refused to give up/but he refused to give upDialogue'));
			case 'the slaughter countinues':
				dialogue = CoolUtil.coolTextFile(Paths.txt('the slaughter countinues/the slaughter countinuesDialogue'));
			case 'an enigmatic encounter':
				dialogue = CoolUtil.coolTextFile(Paths.txt('an enigmatic encounter/an enigmatic encounterDialogue'));
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'not a slacker anymore' | 'but he refused to give up' | 'the slaughter countinues' | 'an enigmatic encounter':
		    {
		            defaultCamZoom = 0.7;
					curStage = 'lastCorridor';
					var bg:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('lastCorridorBG'));
					bg.antialiasing = true;
